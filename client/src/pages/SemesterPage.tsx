...
import { BookOpen, FileText, CheckCircle2, ChevronDown, ChevronUp } from "lucide-react";
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          <div className="space-y-4">
            {filteredCourses.map((course) => { const isExpanded = expandedId === course.id; return <div key={course.id} className={`bg-white border rounded-2xl overflow-hidden transition-all duration-200 ${isExpanded ? "border-cyan-300 shadow-lg shadow-slate-200" : "border-slate-200 hover:border-cyan-300"}`}>
              ...
              {isExpanded && <div className="p-6 sm:p-7 pt-0 border-t border-slate-100 space-y-6 animate-in fade-in duration-300"><p className="text-slate-600 leading-relaxed pt-6">{course.deskripsi}</p><div className="grid grid-cols-1 md:grid-cols-2 gap-5"><div className="bg-slate-50 border border-slate-200 p-5 rounded-xl"><h3 className="text-xs font-bold text-blue-700 uppercase tracking-wider flex items-center gap-2 mb-3"><CheckCircle2 className="w-4 h-4" /> Hal yang dipelajari</h3><ul className="space-y-2 text-sm text-slate-600">{course.halDipelajari.map((item, idx) => <li key={idx} className="flex items-start gap-2"><span className="w-1.5 h-1.5 rounded-full bg-cyan-500 mt-2 shrink-0" />{item}</li>)}</ul></div><div className="bg-slate-50 border border-slate-200 p-5 rounded-xl"><h3 className="text-xs font-bold text-blue-700 uppercase tracking-wider flex items-center gap-2 mb-3"><FileText className="w-4 h-4" /> Tugas / proyek</h3><ul className="space-y-2 text-sm text-slate-600">{course.tugas.map((item, idx) => <li key={idx} className="flex items-start gap-2"><span className="w-1.5 h-1.5 rounded-full bg-blue-500 mt-2 shrink-0" />{item}</li>)}</ul></div></div><div className="bg-cyan-50 border border-cyan-100 p-5 rounded-xl"><h3 className="text-xs font-bold text-cyan-800 uppercase tracking-wider mb-2">Refleksi pribadi</h3><p className="text-slate-700 text-sm italic leading-relaxed">"{course.refleksi}"</p></div><div><h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Bukti / dokumentasi</h3><div className="flex flex-wrap gap-2">{course.dokumentasi.map((doc, idx) => <span key={idx} className="px-3 py-1.5 rounded-lg bg-white border border-dashed border-slate-300 text-slate-500 text-xs">{doc}</span>)}</div></div></div>}
            </div>; })}
          </div>
        </div>
      </main>
